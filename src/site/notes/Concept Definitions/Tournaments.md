...
Note that because $deg^-(A) = 0$, no other vertex can come before $A$ in any valid ranking. If there were another vertex $F$, such that $v \to F$ for any vertex $v \in T$, then $F$ must be the final vertex in the ranking.
