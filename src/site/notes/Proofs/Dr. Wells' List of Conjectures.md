...
5. For any even integer n ≥ 4, the cycle on n vertices, Cₙ, is bipartite.

6. For any graph G, if Δ(G) ≥ 2, then G has at least 3 vertices.

7. For any graph G, |V| ≥ Δ(G) + 1.

8. For any graph G, if δ(G) ≥ 2 (all the vertices have degree at least 2), then G contains a cycle.

9. If T is a tree with n vertices, then T has n − 1 edges.

10. In any tree T, for any vertices u and v, there is exactly one path from u to v.
...
16. Let G = (V,E) be a graph with |V| = n. If |E| ≥ n, then G has (at least one) cycle.

17. For every positive integer n, if T is a tournament with n vertices then there is at least one admissible ranking. That is, there is a sequence of vertices v₁, v₂, …, vₙ₋₁, vₙ so vᵢ → vᵢ₊₁ for i = 1, 2, …, n − 1.

18. Let T be a tournament and C be cycle in T. If C is maximal, then for every vertex v of T that is not in C, either for every vertex c in C, v → c, or for every vertex c in C, c → v.
